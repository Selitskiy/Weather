function generic_test_in_graph2D(M, l_m, X, Y, X2, Y2, Sx, Sy, Sx2, Sy2, l_y, n_sess, l_sess, k_ob, x_in, t_in, y_out, t_out, k_tob, t_sess, sess_off, offset, k_start, model_name)
    ...
    for i = 1:n_sess
        ...
        for j = 1:k_ob
            Mxw = reshape(X(:, j, i), [x_in, t_in])';
            for k = 1:x_in
                %lp = plot(Sx(1,j,i):Sx(2,j,i), Mxw(:,k), 'g', 'LineWidth', 2);
                M2(Sx(1,j,i):Sx(2,j,i),k) = Mxw(:,k);
            end
        end

            for k = 1:x_in
                lp = plot(x_st:x_end, M2(x_st:x_end,k)+1, 'g','LineWidth', 2);
                hold on;
            end
    end
    ...
    for i = 1:t_sess-sess_off
        ...
        for j = 1:k_tob
            ...
            for k = 1:x_in
                ...
                M3(Sx2(1,j,i):Sx2(2,j,i),k) = Mxw(:,k);
            end
        end

            for k = 1:x_in
                lp = plot(x_st2:x_end2, M3(x_st2:x_end2,k)-1, 'm','LineWidth', 2);
                hold on;
            end
    end
    ...
    for i = 1:n_sess
        ...
        for j = 1:k_ob
            ...
            for k = 1:y_out
                ...
                M2(Sy(1,j,i):Sy(2,j,i), x_in+k) = Myw(:,k);
            end
        end

            for k = 1:y_out
                lp = plot(y_st:y_end, M2(y_st:y_end, x_in+k)+1, 'c','LineWidth', 2);
                hold on;
            end
    end
    ...
    for i = 1:t_sess-sess_off
        ...
        for j = 1:k_tob
            ...
            for k = 1:y_out
                %lp = plot(Sy(1,j,i):Sy(2,j,i), Myw(:,k)+1, 'm', 'LineWidth', 2);
                M3(Sy2(1,j,i):Sy2(2,j,i), x_in+k) = Myw(:,k);
            end
        end

            for k = 1:y_out
                lp = plot(y_st2:y_end2, M3(y_st2:y_end2, x_in+k)+1, 'r','LineWidth', 2);
                hold on;
            end
    end
